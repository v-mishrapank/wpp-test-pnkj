...
- Teams dispatcher, email dispatcher, and bot endpoint Function Apps
...
Set `environment` to the matching map key for each deployment. Store the real IDs in environment-specific tfvars files or pipeline variables rather than committing them to source control.
